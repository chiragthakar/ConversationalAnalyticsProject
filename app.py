...
with st.sidebar:
    ...
    enable_debug = st.checkbox("🐞 Enable Debugger Panel", value=True, help="Show raw LLM prompts, context payloads, and BigQuery execution metadata.")
...
if run_query and user_query.strip():
    ...
    with st.spinner("Executing on GCP BigQuery Engine..."):
        try:
            ...
            # Developer Debugger Panel Render
            if enable_debug:
                st.markdown("---")
                st.markdown("### 🐞 Developer Debugger Panel")
                dbg_tab1, dbg_tab2, dbg_tab3 = st.tabs(["📝 LLM Context Prompt Sent", "⚡ Raw Agent Output JSON", "🔍 Graph Topology Payload"])
                
                with dbg_tab1:
                    raw_context = kc_service.get_full_context_prompt(user_query)
                    st.caption("Complete System Context Prompt assembled from live GCP BigQuery schemas & Dataplex Catalog entries:")
                    st.code(raw_context, language="markdown")

                with dbg_tab2:
                    st.caption("Raw JSON response generated by Agent:")
                    st.json(agent_response)

                with dbg_tab3:
                    st.caption("Parsed Nodes & Edges payload for graph visualizer:")
                    st.json(graph_meta)
